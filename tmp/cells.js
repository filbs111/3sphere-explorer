...
function addMatsFromMat(thisMat){
	...
	//sort arrays by distance from portal. makes rendering faster
	...
	var d600reverse = returnObj.d600.slice(0).reverse();
	returnObj.d600reverse=d600reverse;
	
	//make 8 lists. guess should be array. to decide which list to use : 
	// check which is greatest abs(x), abs(y)
	//this calculation doesn't need to be efficient since only running once (main view) or twice (+cubemap) per frame
...
function sortIdForMatrix(inputMatrix){
	...
	*/
	...
	return greatestIdx + (position[greatestIdx] > 0 ? 4 : 0);
	
	//TODO just show on screen or something which 8-cell are in
}

var cellMatDataSortable = (function(){
	//maybe a crappy system. want to sort matrices in draw orderin camera frame.
	//might be better to stick together with unrotated mats
	
	var toReturn={};
	
	toReturn.d5 = emptyMatsArray[5];
	toReturn.d8 = emptyMatsArray[8];
	toReturn.d16 = emptyMatsArray[16];
	toReturn.d24 = emptyMatsArray[24];
	toReturn.d120 = emptyMatsArray[120];
	toReturn.d600 = emptyMatsArray[600];

	return toReturn;
	
	function emptyMatsArray(num){
		/*
		var theArray=[];
		for (var ee=0;ee<num;ee++){
			theArray.push(mat4.create());
		}
		*/
		
		var theArray=new Array(num);
		for (var ee=0;ee<num;ee++){
			theArray[ee]=mat4.create();	//array.push faster? todo compare
		}
	}
