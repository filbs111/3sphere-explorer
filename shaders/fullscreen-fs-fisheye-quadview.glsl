...
uniform vec2 xMultShiftCopy;
uniform vec2 yMultShiftCopy;
...
//vec2 modifiedTextureCoord = 2.*vTextureCoord/(uOversize*uInvF);; // guess bollocks....
    ...
    //bodge addition to modify shader intended to use whole texture (generated by single previous rectilinear render)
    // to only read from a quarter of it. TODO refactor maths to not require fs uniforms, do logic in vert shader.
    adjustedSampleCoords*=vec3(xMultShiftCopy.x, yMultShiftCopy.x, 2.); //mult
    adjustedSampleCoords-=1.*adjustedSampleCoords.z*vec3(xMultShiftCopy.y, yMultShiftCopy.y,0.);   //shift
