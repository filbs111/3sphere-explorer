    ...
    // vDebugColor = vec4(1.0,0.0,uDebugDarkeningMultiplier,1.0);
		...
		for (float aa=0.;aa<CONST_ITERS;aa++){
			...
			if (abs(modifiedSinStart - modifiedSinEnd)<0.000001){
				...
				sumb+= (endDensity-startDensity)/(modifiedSinEnd - modifiedSinStart);
			}
		}
		sumb/=uAtmosContrast;

		sumb*= bodgeScale*maxDoubleAngOverIters*exp(uAtmosContrast*(constTerm));	
		fog = exp(-uAtmosThickness*sumb/2.0);
#endif
#ifdef ATMOS_ONE_OLD
		float maxDoubleAngOverIters = maxDoubleAng/CONST_ITERS;
